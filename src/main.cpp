...
// Clusters a single place into 2 different sub-places
...
void placeCallback(std_msgs::Int16 placeId)
{
    ...
    //for (int i=0; i < aPlace.memberIds.rows; i++)
    //    std::cout << "memberIds : " << aPlace.memberIds.at<int>(i) << endl;

//    for(int i = 0; i < ncols; i++)
//    {
//        if(clusterid[i] == 0)
//        {
//            divPlace1.id = aPlace.id;
//            divPlace1.memberIds.at<int>(m) = aPlace.memberIds.at<int>(m);
//            divPlace1.memberInvariants.at<double>(m) = aPlace.memberInvariants.at<double>(m);
//            qDebug() << "No problem till here 3";
//            m++;
//        }
//        else
//        {
//            divPlace2.id = aPlace.id +1;
//            //divPlace2.memberIds.at<int>(n) = aPlace.memberIds.at<int>(n);
//            divPlace2.memberInvariants.at<double>(n) = aPlace.memberInvariants.at<double>(n);
//            n++;
//        }
//    }
...
void clusterPlace(Place pl)
{
    //Place divPlace1,divPlace2;
    //divPlace1.memberIds = Mat::zeros(pl.memberIds.size(),pl.memberIds.type());
    //divPlace2.memberIds = Mat::zeros(pl.memberIds.size(),pl.memberIds.type());
    Mat currentInvariants = pl.memberInvariants;
    Mat transposeInvariants;
    transpose(currentInvariants,transposeInvariants);
    int nrows = transposeInvariants.rows;
    int ncols = transposeInvariants.cols;
    ...
    for(int i=0; i < nrows;++i)
    {
       // weight[i] = 1.0;
        ...
        {
            data[i][j] = transposeInvariants.at<float>(i,j);
            mask[i][j] = 1;
        }
    }
    ...
    //Node* placeHierarchy = treecluster(nrows,ncols,data,mask,weight,1,'e','s',distmatrix);
    //qDebug() << "No error till here \n";
    Node* placeHierarchy = treecluster(nrows,nrows,0,0,0,0,'e','s',distmatrix);

    //int clusterid[ncols];
    //cuttree(ncols,placeHierarchy,2,clusterid);

    std::vector<TreeLeaf> leaves;
    //qDebug() << "Column size: " << ncols << "\n";
    ...
    for(int i = 0 ; i < nrows; i++)
    {
        ...
        // The left child
        leaf.left = placeHierarchy[i].left;
        ...
        if(leaf.left < 0) leaf.left = nrows-leaf.left -1;

        // The right child
        leaf.right = placeHierarchy[i].right;
        ...
        if(leaf.right < 0) leaf.right= nrows-leaf.right -1;

        // The value of the leaf
        leaf.val = placeHierarchy[i].distance;
        //qDebug() << "distance"<<placeHierarchy[i].distance << "\n";

        leaves.push_back(leaf);
    }

    TreeLeaf currentLeaf;
    qDebug("Size of the place = %d x %d \n",nrows,ncols);
    placeTreeStream << "Clusters for the place:" << pl.id << "\n";

    for(int i=0;i < leaves.size()-1;i++)
    {
        currentLeaf = leaves.at(i);
        if(currentLeaf.right < ncols && currentLeaf.left < ncols)
            placeTreeStream<<currentLeaf.left+1<<" "<<currentLeaf.right+1<<" "<<currentLeaf.val<<" "<<"\n";
        else if(currentLeaf.right < ncols)
            placeTreeStream<<currentLeaf.left<<" "<<currentLeaf.right+1<<" "<<currentLeaf.val<<" "<<"\n";
        else if(currentLeaf.left < ncols)
            placeTreeStream<<currentLeaf.left+1<<" "<<currentLeaf.right<<" "<<currentLeaf.val<<" "<<"\n";
        else
            placeTreeStream<<currentLeaf.left<<" "<<currentLeaf.right<<" "<<currentLeaf.val<<" "<<"\n";
    }

//    MatConstIterator_<double> it_begin = currentInvariants.begin<double>();
//    MatConstIterator_<double> it_end = currentInvariants.end<double>();

//    double* p;

//    for(int i=0; i < ncols ; ++i)
//    {
//        p = transposeInvariants.ptr<double>(i);
...
//    for (MatConstIterator_<double>it = it_begin; it !=it_end;++it)
//        placeTreeStream << *it << "\n";
...
//        for(int j=0; j < i ; ++j)
//        {
//            placeTreeStream << distmatrix[i][j] << " ";
//        }
//        placeTreeStream << "\n";
//    }


    //currentPlaces.push_back(divPlace1);
    //currentPlaces.push_back(divPlace2);
}
...
//    }
    ...
    /*for (int i=0; i<bdst->levels.size(); i++){
        std::cout << "bdst level " << i << " invariant: " << std::endl;
        for (int j=0; j<bdst->levels.at(i).meanInvariant.size(); j++){
            qDebug() <<  bdst->levels.at(i).meanInvariant.at(j) ;
        }
    }*/
